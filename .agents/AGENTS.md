...
6. **Open Funds**: Use the keyless Fmarket adapter for the open-fund universe and NAV. Persist the Fmarket product ID in `MarketAsset.ExternalId`, keep NAV in absolute VND, import idempotently, and preserve the last NAV with `Stale` status when Fmarket fails.
