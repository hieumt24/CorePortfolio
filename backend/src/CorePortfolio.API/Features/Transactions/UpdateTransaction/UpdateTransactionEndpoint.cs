...
public record UpdateTransactionRequest(TransactionType Type, decimal Quantity, decimal Price, string? Currency, DateTime? Timestamp);
...
public static class UpdateTransactionEndpoint
{
    public static void MapUpdateTransactionEndpoint(this IEndpointRouteBuilder app)
    {
        ...
        {
            var command = new UpdateTransactionCommand(id, request.Type, request.Quantity, request.Price, request.Currency, request.Timestamp);
