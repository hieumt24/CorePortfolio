...
- Transaction file transfer: the `/transactions` ledger now supports user-scoped client-side CSV, SpreadsheetML XLS, and paginated PDF exports, plus imports from those formats (including duplicate-ID skipping and portfolio/asset resolution by ID or name). No new backend endpoint or schema is required; writes continue through the existing validated transaction command.
- Transaction bulk deletion: authenticated users can delete their complete Crypto, Stock, and Fund/ETF transaction history through `DELETE /api/transactions`. The operation is user-scoped and atomic, removes linked cash-ledger entries and transaction-generated cashflows, and preserves portfolios, assets, and cash accounts.
