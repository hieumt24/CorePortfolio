...
- Transaction tracking UX: `/transactions` now groups the global ledger into Crypto, Cổ phiếu, and CCQ/ETF tabs (with Vietnamese/English category matching), quick counts, and a category-aware add-transaction entry point. The add modal shows loading feedback while assets/categories are being resolved. The existing user-scoped transaction API contract remains unchanged.
